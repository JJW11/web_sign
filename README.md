...
2. 관절이 표시된 카메라 화면과 실시간 단어 예측률 확인, 웹 오른쪽 상단에 WS(WebSocket) 상태 connected 확인 -> 정상 작동
3. 수어 동작 수행 → 임계값 초과 시 단어가 버퍼에 자동 등록
4. 원하는 단어들이 모두 등록되면 **Translate** 클릭
5. 생성된 문장 확인 및 TTS 음성 출력
6. **Reset** 클릭 -> 현재 버퍼의 단어들 초기화
7. 프리셋 기능 확인 (where.mp4:"어디가 아프세요?" /since.mp4:"언제부터 아프셨어요?" /help.mp4:"무엇을 도와드릴까요?")
---
